...
export class CdkDragDropConnectedSortingGroupExample {
  ...
  todo = this.d.diadiem;
  thoigian = this.d.thoigian[this.inde];
  thien: any = [];
  add: boolean = false;
  hih: boolean = false;
  ...
  drop(event: CdkDragDrop<any>) {
    ...
    if (event.previousContainer === event.container) {
      ...
      this.vuong[this.inde]=event.container.data
    } else {
      transferArrayItem(
    ...
    {
      this.vuong[this.inde]=event.previousContainer.data
    }
        ...
        if (event.currentIndex == 0) {
          for (let i = 1; i < this.thoigian.length; i++) {
            ...
            this.thoigian[i].timeE =
              result.getHours().toLocaleString() +
              ':' +
              result.getMinutes().toLocaleString().padStart(2, '0');
            console.log(this.thoigian);
          }
  ...
  location=this.locations[this.inde]
  ...
  thi(i:any)
{
    ...
    this.location=this.locations[this.inde]
  ...
  update()
  {
    this.d.vitri[this.inde]=[]
    this.tam=[]
    console.log(this.d.tong.length)
    this.mot=this.d.tong[0][0].lat
    this.hai=this.d.tong[0][0].lng
    this.loading=false
    for (let i = 0; i < this.d.tong.length; i++) {
      ...
      for (let j = 1; j < this.d.tong[i].length-1; j++) {
   
      this.tam[i].push(
        {
          "_id":this.vuong[i][j].id
        }
      )
    ...
    instance
    .patch(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/routes/${this.d.index}?checked=true`,
    {
      "routes": this.tam,
    }
    )
    .then((response) => {
          ...
          this.loading=true
          this.router.navigateByUrl('/itinerary')
          this.d.extraitinerary[this.inde]=this.vuong[this.inde]
          console.log(this.d.extraitinerary)   
    ...
    .catch( (error) => {
      if(confirm(`${error.response.data.message} Do you want to continue updating?`) == true){
        instance
        .patch(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/routes/${this.d.index}?checked=false`,
        {
          "routes": this.tam,
        }
        )
  ...
  navigation(typ:any,i: any) {
    ...
    document
      .querySelectorAll('.total-navigation_button')
      [i].classList.add('choice');
    this.diem = typ;
    this.inn=i
    
    instance
    .get(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/locations?&page=${this.diem}&take=20`)
    .then((response) => {
...
reset()
{
  ...
  this.d.tong=[]
  instance
  .get(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/locations?&page=1&take=20`)
  .then((response) => {
    console.log(response.data);
...
if(!this.d.itinerary.routes[0].length)
{
  ...
  for (let j = 0; j < this.d.itinerary.routes.length; j++)
      ...
      for (let i = 0; i < this.d.tong.length; i++) {
        for (let j = 0; j < this.d.itinerary.routes[0].route.length; j++) {
          
        this.d.tong[i].push
        (
          {
            name:this.d.itinerary.routes[i].route[j].description.name,
            img:this.d.itinerary.routes[i].route[j].description.photos,
            address:this.d.itinerary.routes[i].route[j].description.address,
            timeS:this.d.itinerary.routes[i].route[j].travelTime.arrival,
            timeE:this.d.itinerary.routes[i].route[j].travelTime.departure,
            id:this.d.itinerary.routes[i].route[j].description._id,
            lat:this.d.itinerary.routes[i].route[j].description.latitude,
            lng:this.d.itinerary.routes[i].route[j].description.longitude
...
else if(this.d.itinerary.routes[0].length)
{
  console.log('quần')
  for (let j = 0; j < this.d.itinerary.routes.length; j++)
      {
      ...
      for (let i = 0; i < this.d.tong.length; i++) {
        for (let j = 0; j < this.d.itinerary.routes[0].length; j++) {
          
        this.d.tong[i].push
        (
          {
            name:this.d.itinerary.routes[i][j].name,
            img:this.d.itinerary.routes[i][j].photos,
            address:this.d.itinerary.routes[i][j].address,
            timeS:this.d.itinerary.routes[i][j].timeS,
            timeE:this.d.itinerary.routes[i][j].timeE,
            id:this.d.itinerary.routes[i][j]._id,
            lat:this.d.itinerary.routes[i][j].latitude,
            lng:this.d.itinerary.routes[i][j].longitude
...
generate()
{
  ...
  instance
  .post(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/routes/${this.d.index}/generate`,
  {
    "routes": this.tam,
  }
  )
  ...
  this.d.extraitinerary=[]
  this.d.diadiem=[]
  this.d.tong[this.inde]=[]
  this.d.thoigian[this.inde]=[]
  this.d.vitri[this.inde]=[]
  instance
  .get(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/locations?&page=1&take=20`)
  .then((response) => {
  ...
  console.log(response.data.data.routes);
    ...
    this.d.thoigian[this.inde].push
    (
      {
        timeS:response.data.data.routes[this.inde].route[j].travelTime.arrival,
        timeE:response.data.data.routes[this.inde].route[j].travelTime.departure
        ...
        this.thoigian = this.d.thoigian[this.inde];
...
search(value: any) {
  this.d.diadiem=[]
    instance
      .get(
        `http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/locations?keyword=${value.keyword}&page=1&take=16`
      )
      .then((response) => {
        for (let i = 0; i < response.data.listLocations.length; i++) {
          if(response.data.listLocations[i].photos)
...
//   instance
//     .get(
//       `http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com/locations?keyword=${value.target.value}&page=1&take=16`
//     )
//     .then((response) => {
//       for (let i = 0; i < response.data.listLocations.length; i++) {
//         if(response.data.listLocations[i].photos)
