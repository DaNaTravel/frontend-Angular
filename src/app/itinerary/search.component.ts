...
export class Search {
  ...
  type=['Art','Historical','Cuisine','Relax', 'Natural']
  ...
  ngOnInit() {
    ...
    this.hai=this.d.lng
    instance
    .get(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com:5000/locations?&page=1&take=20`)
    .then((response) => {
...
this.loading=false
instance
    .post(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com:5000/routes`,
    {
      "latitude":this.mot,
      "longitude":this.hai,
      "startDate":start,
    ...
    .then((response) => {
      ...
      this.d.location=this.d.itinerary.routes
      console.log(response);
      this.loading=true
      
      for (let j = 0; j < response.data.data.routes.length; j++)
      {
        this.d.tong.push([])
        this.d.thoigian.push([])
        this.d.vitri.push([])
      }
      console.log(response.data.data.routes.length);
      console.log(this.d.tong);
      console.log(this.d.thoigian);
      console.log(this.d.vitri);
      
      
      for (let i = 0; i < response.data.data.routes.length; i++) {
        for (let j = 0; j < response.data.data.routes[i].route.length; j++) {
...
this.currentDate=value
  ...
  navigation(typ:any,i: any) {
    ...
    instance
    .get(`http://ec2-13-114-139-244.ap-northeast-1.compute.amazonaws.com:5000/locations?&page=${this.diem}&take=20`)
    .then((response) => {
      this.locations=response.data.listLocations
    })
